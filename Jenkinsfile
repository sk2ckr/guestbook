...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="sk2ckr/cicd_guestbook:${strDockerTag}"
    }
    ...
    stages {
        stage('Checkout') {
            agent { label 'agent1' }
            steps {
                git branch: 'master', url:'https://github.com/sk2ckr/guestbook.git'
            }
        }
        stage('Build') {
            agent { label 'agent1' }
        ...
        stage('Docker Image Push') {
            ...
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_sk2ckr') {
                        oDockImage.push()
                    }
                }
            }
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'sk2ckr.j@gmail.com')
